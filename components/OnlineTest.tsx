...
const OnlineTest: React.FC = () => {
  ...
  // Update available subjects based on grade
  useEffect(() => {
    let subjects: Subject[] = [];
    const grade = config.grade;

    if (grade === Grade.GRADE_1 || grade === Grade.GRADE_2) {
      // Grade 1, 2: Math, Nature & Society
      subjects = [Subject.MATH, Subject.NATURE_SOCIETY];
    } else if (grade === Grade.GRADE_3) {
      // Grade 3: Math, Nature & Society + IT, Tech
      subjects = [
        Subject.MATH, 
        Subject.NATURE_SOCIETY,
        Subject.IT,
        Subject.TECH
      ];
    } else if (grade === Grade.GRADE_4 || grade === Grade.GRADE_5) {
      // Grade 4, 5: All subjects
      subjects = [
        Subject.MATH, 
        Subject.IT, 
        Subject.TECH, 
    ...
    // If current subject is not in new list, reset to first available
  ...
  const calculateScore = () => {
    ...
    // Scoring Logic:
    // IT/Tech: 16 MCQs + 2 Complex. Total 10. (approx 0.5 per MCQ, 1 per complex)
    // English: 40 MCQs. Total 10. (0.25 per MCQ)
    // Others: 20 MCQs. Total 10. (0.5 per MCQ)
      ...
      if (q.type === 'MCQ') {
         if (userAns === q.correctAnswer) {
             if (isEnglish) {
               ...
               // IT/Tech and Others (Math, etc) both ~0.5 per MCQ
               score += 0.5;
             }
         }
      } else {
         // Matching or Fill-in (Only for IT/Tech)
         // Simplified scoring: 1.0 point if answered (Should verify correctness in real app)
         if (userAns && userAns.length > 0) {
           score += 1.0; 
         }
      }
    });
    return Math.min(score, 10); // Cap at 10 just in case
  };
      ...
      <div className="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="space-y-4">
           <div>
            ...
            <select className="w-full p-3 border rounded-lg" value={config.grade} onChange={(e) => setConfig({...config, grade: e.target.value as Grade})}>
              {Object.values(Grade).map(g => <option key={g} value={g}>{g}</option>)}
            </select>
           </div>
           <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Môn</label>
            <select className="w-full p-3 border rounded-lg" value={config.subject} onChange={(e) => setConfig({...config, subject: e.target.value as Subject})}>
           ...
           {quizData.questions.map((q, idx) => (
             <div key={q.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex gap-3">
                   <span className="bg-blue-100 text-blue-800 font-bold px-3 py-1 rounded h-fit whitespace-nowrap">Câu {idx + 1}</span>
                   <div className="flex-1">
                      ...
                      {q.type === 'MATCHING' && (
                         <div className="bg-orange-50 p-4 rounded border border-orange-200 text-sm">
                            <p className="font-bold text-orange-800 mb-2">Hãy nối ý cột A với cột B:</p>
                            {q.matchingPairs?.map((pair, i) => (
                               <div key={i} className="grid grid-cols-2 gap-4 mb-2">
                                  <div className="bg-white p-2 rounded border border-orange-200">{pair.a}</div>
                                  <div className="bg-white p-2 rounded border border-orange-200">{pair.b}</div>
                               </div>
                            ))}
                            <textarea 
                             className="w-full mt-2 p-3 border rounded bg-white text-gray-800"
                             rows={2}
                             placeholder="Nhập câu trả lời của bạn (Ví dụ: 1-b, 2-a...)"
                             onChange={(e) => handleAnswer(q.id, e.target.value)}
                           />
                         </div>
                      )}

                      {q.type === 'FILL_IN' && q.fillInParts && (
                        <div className="bg-green-50 p-4 rounded border border-green-200 text-sm">
                           {q.fillInParts[0].wordBank && (
                             <div className="mb-3">
                               <span className="font-bold text-green-800">Từ gợi ý: </span>
                               <span className="italic">({q.fillInParts[0].wordBank.join(", ")})</span>
                             </div>
                           )}
                           <div className="bg-white p-3 rounded border border-green-200 mb-2">
                              {q.fillInParts[0].text}
                           </div>
                           <textarea 
                             className="w-full mt-2 p-3 border rounded bg-white text-gray-800"
                             rows={2}
                             placeholder="Nhập các từ cần điền theo thứ tự..."
                             onChange={(e) => handleAnswer(q.id, e.target.value)}
                           />
                        </div>
                      )}
